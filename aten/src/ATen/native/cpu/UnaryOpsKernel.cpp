...
static void sigmoid_kernel(TensorIteratorBase& iter) {
  ...
  if (dtype == at::kFloat && iter.is_contiguous()) {
    ...
    int64_t i = 0;
    for (; i + static_cast<int64_t>(vlmax) <= n; i += static_cast<int64_t>(vlmax)) {
      vfloat32m2_t x = __riscv_vle32_v_f32m2(&in_ptr[i], vlmax);
      vfloat32m2_t exp_neg_x = Sleef_expfx_u10rvvm2(__riscv_vfneg_v_f32m2(x, vlmax));
      vfloat32m2_t one = __riscv_vfmv_v_f_f32m2(1.0f, vlmax);
      vfloat32m2_t result = __riscv_vfrdiv_vf_f32m2(__riscv_vfadd_vv_f32m2(one, exp_neg_x, vlmax), 1.0f, vlmax);
      __riscv_vse32_v_f32m2(&out_ptr[i], result, vlmax);
    }
    for (; i < n; ++i) {
